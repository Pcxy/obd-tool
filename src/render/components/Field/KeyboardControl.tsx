...
import { connect, Gear, KeyStatus, OBDGPSStateType, StatsStateType } from 'umi';
...
export default (props: Props) => {
  const { obdgps, stats, dispatch } = props;
  ...
  useEffect(() => {
    ...
    const sendOBDToMainInterval = setInterval(sendOBDToMainCallback, 1000 / obdRate);
    const sendGPSToMainInterval = setInterval(sendGPSToMainCallback, 1000 / gpsRate);
    return () => {
  ...
  const renderIntervalCallback = () => {
    ...
    const { pause } = latestStats.current;
